...
\begin{itemize}
  \item \textbf{3D LiDAR.} A rotating 3D LiDAR operating at $33.6$\,Hz
        (scan period $\approx 30$\,ms), producing dense point clouds published
        as \texttt{sensor\_msgs/PointCloud2}.

  \item \textbf{Stereo camera.} A Leopard Imaging LI-AR0234CS-NVIDIA Hawk
  ...
  \item \textbf{IMU.} An inertial measurement unit integrated with the camera
        module, publishing linear acceleration and angular velocity at
        approximately $62$\,Hz as \texttt{sensor\_msgs/Imu}.

  \item \textbf{Wheel odometry.} Pose increments derived from motor encoders,
        published as \texttt{nav\_msgs/Odometry}.
\end{itemize}
