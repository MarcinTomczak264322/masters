...
the belief is uniform over the entire map and converges to true pose with more observations.
...
Two problem formulations are common~\cite[pp.~245--246]{thrun2005probabilistic}.
...
\end{equation}
Full SLAM keeps all past poses and estimates the complete trajectory
together with the map:
\begin{equation}
...
A limitation is motion distortion: as the robot moves continuously during a
scan, successive points are captured from slightly different poses, making
...
These are published as \texttt{sensor\_msgs/Imu} messages at a higher rate
than other sensors, providing motion information between slower LiDAR
or camera updates.
However, integrating corrupted signals accumulates errors over time, so the
IMU must be combined with another sensor to remain accurate.
...
\texttt{nav\_msgs/Odometry} message containing position and orientation.
Wheel odometry is a useful baseline for short-term motion estimation.
...
The wide coverage reduces the risk of losing tracked features during sharp
rotations and provides richer context for place recognition.
The main drawbacks are strong lens distortion and the absence of a single
agreed sensor model, since the term covers fisheye, catadioptric, and
