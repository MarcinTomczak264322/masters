...
Any pose error further degrades the result, since a measurement placed at
the wrong location introduces geometric inconsistencies that propagate across
the entire model.
...
Three operating modes are distinguished depending on prior knowledge~\cite[p.~159]{thrun2005probabilistic}.
In \emph{pose tracking} a reliable initial estimate is already available and the
uncertainty remains small throughout operation.
In \emph{global localisation} the starting position is completely unknown, so the
belief may be spread across the whole environment and can be multimodal.
The \emph{kidnapped robot problem} is a harder variant of global localisation:
the robot is teleported to an unknown location during operation without being
informed, so it must detect that its current belief is wrong and recover a
correct estimate from scratch.
In this thesis the algorithms operate in passive mode --- they process sensor
data recorded along a fixed trajectory without influencing the robot's motion.

\subsection{SLAM}

SLAM removes the assumption of known poses from mapping and the assumption of a
known map from localisation, solving both problems jointly from raw sensor
measurements $\mathbf{z}_{1:t}$ and motion commands $\mathbf{u}_{1:t}$
alone~\cite{thrun2005probabilistic}.
...
% TODO: Section intro
%   - Explain the two-phase structure: front-end (build graph) and back-end (optimise graph)
%   - Contrast with filter-based: graph retains full trajectory, enables global consistency
