...
Here add why these algorithms were chosen.
...
($x$, $y$, $\theta$). This simplification lowers computational cost and is valid for wheeled robots
on flat floors, but it discards all
...
Loop closure detection is appearance-based: compact descriptors of each scan
are compared against all stored nodes, enabling detection of revisits after
long detours (Section~\ref{sec:loop_closure}).
...
Two systems are evaluated, both using a stereo camera pair.
